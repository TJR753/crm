...
<script type="text/javascript">
	...
	$(function(){
		...
		getActivityList("${clue.id}")
		//关联市场活动的模糊查询
		//"text":$("#searchActivity").val(),"id":clueId
		$("#searchActivity").keydown(function (event){
			if(event.keyCode==13){
				$.ajax({
					url:"workbench/clue/getActivityListAndExcludeClueId.do",
					data:{
						"searchText":$.trim($("#searchActivity").val()),
						"id":"${clue.id}"
					},
					dataType:"json",
					type:"get",
		...
		$("#linkedBtn").click(function(){
			...
			if(ids.length==0){
				...
				for(var i=0;i<ids.length;i++){
					var clueId="${clue.id}"
					var activityId=$(ids[i]).val()
					linkedActivity(clueId,activityId);
				}
			}
		})
	});
		...
		$.ajax({
			...
			success:function (data){
				...
				$.each(data,function (i,n){
					...
					html+='<td>'+n.endDate+'</td>'
					html+='<td>'+n.name+'</td>'
					html+='<td><a href="javascript:void(0);"  style="text-decoration: none;" onclick="disassociate(\''+n.id+'\')"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>'
					html+='</tr>'
				})
	...
	//关联函数
	function linkedActivity(clueId,activityId){
		$.ajax({
			url:"workbench/clue/linkedActivity.do",
			data:{
				"clueId":clueId,
...
<%--							</tr>--%>
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" id="linkedBtn">关联</button>
				</div>
			</div>
		</div>
	</div>
